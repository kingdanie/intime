    ...
    <OtherLayout>
      ...
      >
        ...
        <section className="flex flex-col items-center border border-dotted border-gray-500 rounded-lg my-5 p-5">
            <div className="flex items-center"><MagnifyingGlassIcon className="h-5"/> <h3>Point conversion and meessage costs</h3></div>
            <div className="mt-10 mb-2 h-0 w-4/5 border border-solid border-gray-400"></div>
            <div className="w-full">
                {[{name: " 1 Naira", points: "1"}, {name: "  Naira", points: "4"}, {name: "Single scheduled sms", points: "8"}, {name: "Bulk Instant sms", points: "6"}, {name: "Bulk Scheduled sms", points: "12"}].map(plan => (
                <div key={plan.name} className="grid grid-cols-12 my-5 gap-4 border border-solid border-gray-300 rounded-lg px-3 py-1 w-full">
                <div className="col-span-5"><span>{plan.name}</span></div>
                <div className="col-span-2 self-center justify-self-center text-center"> <Image width={60} height={60} src={PricingArrows} alt="pricing arrows"></Image> </div>
                <div className="col-span-5 text-end"><span>{plan.points} Point{(Number(plan.points) > 1) ? 's' : null}</span></div>
        </div>
                ))}
        ...
        <section className="grid md:grid-cols-10 md:p-10 gap-10 md:space-y-0">
            ...
            <div className="flex items-center justify-center rounded-full p-2 bg-yellow-400 w-10 h-10">
                <Image src={Transfer} width={20} height={20} alt="transfer" ></Image>
            </div>
